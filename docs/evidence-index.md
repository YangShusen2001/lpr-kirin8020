...
| 3.4 | RQ4 端到端 p50 / mean / max | **见 §3.1 专节**（20 分钟补跑后更新） | `evidence/rq4_thermal*.csv` | ✅ |
| 3.5 | 张量指纹逐轮恒定：`det.l2` / `rec.l2` / `code` 各只有 1 个取值 | 证明后端计算路径未退化 | `evidence/rq4_thermal.csv` | ✅ |

### 3.1 RQ4 两次采集的条件对照（**引用前必读**）

| 数据集 | 轮数 | 构建 | thermal | 电池 ℃ | 充电 | 说明 |
|---|---|---|---|---|---|---|
| `rq4_thermal_o0_superseded.csv` | 20 | **`-O0`** | 2 | 35–36 | 是 | 已作废，仅作对照 |
| `rq4_thermal.csv` | 25 | `-O2` | **1** | 33–34 | 否 | T7 当前版 |
| `rq4_thermal_release.csv` | 25 | `-O2` | 1 | 33–34 | 否 | **与上一行内容相同**（重复文件） |
| `rq4_thermal_80r.csv` | 80 | `-O2` | **2** | 35 | 是 | 2026-09-21 补跑，20 分钟 |

> **为什么补跑这一版**：T7 的 25 轮全程 `thermal=1`，**从未跨过 thermal 2→3 的台阶**，
> 所以 RQ4 真正要回答的问题（持续负载下 NPU 收益能否维持）**没有被测到**。
>
> **一个意外的收获**：补跑落在 `thermal=2 + 充电中`，**与 `-O0` 旧数据同条件**。
> 于是「构建模式」这个变量第一次被单独隔离出来 —— T7 当时自陈
> 「构建模式、热态档、充电状态三者同时变化，不能严格归因于单一变量」，
> 这一版可以拆开其中两个。
