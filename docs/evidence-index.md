...
| 3.5 | ~~**持续负载退化 +26.9%**~~ → **两次独立重跑均未复现**（−11.5% / +11.2%，方向相反） | 40 轮 / 41 轮各一次，release `-O2` | `evidence/rq4_thermal_runA.csv` + `runB.csv` | ✅ |
| 3.5a | **40 轮尺度上「漂移」与轮间噪声不可区分**：置换检验 p=**0.255** / **0.156** | 20000 次置换；H0 ｜95% 分位｜=20.2% / 14.8% | `tools/permutation_drift_test.py` | ✅ |
| 3.5b | **轮间 CV 约 19–26%**（这是稳定可复现的那一条） | 三次运行：25.5% / 18.7% / 27% | 三个 `rq4_thermal*.csv` | ✅ |
| 3.5c | 热档台阶**真实存在**：运行 B 内 thermal **3→2**，延迟 **79.70 → 74.89 ms（−6.4%）** | ⚠️ 热档与时间/温度共线，**只作观测不作因果** | `evidence/rq4_thermal_runB.csv` | ✅ |
| 3.6 | 张量指纹逐轮恒定：`det.l2` / `rec.l2` / `code` 各只有 1 个取值 | 证明后端计算路径未退化 | `evidence/rq4_thermal*.csv` | ✅ |
| 3.7 | **thermal 2→3 台阶在 80 轮 / 23.3 分钟内未被触发** | 80/80 轮恒为 2；**运行 B 已覆盖台阶两侧**（见 3.5c） | `evidence/rq4_thermal_80r.csv` | ✅ |

> ⚠️ **3.5 已被 #14 推翻，不得再引用「持续负载退化 +26.9%」这个说法。**
> 两次独立重跑（各 40/41 轮）给出 **−11.5%** 与 **+11.2%** —— **方向相反**，
> 且置换检验 p=0.255 / 0.156，**两次都不显著于轮间噪声**（3.5a）。
> 可支持的只有「**轮间 CV 约 19–26%**」（3.5b）与「**热档台阶真实**」（3.5c）。
> 详见 `docs/notes/t14-rq4-two-runs.md`。
>
> ⚠️ **新硬规则（与「禁止 NPU 利用率数字」并列）**：
> 任何形如「A 窗口比 B 窗口慢 X%」的陈述，必须同时给出**同一段数据在 H0（无漂移）下的
> 置换分布**与经验 p 值。p ≥ 0.05 时只能写成「与轮间噪声不可区分」，
> **不得写成「退化 X%」**。守卫：`tools/permutation_drift_test.py`。
...
| `rq4_thermal_80r.csv` | **80** | **23.30 min** | `-O2` | **2** | 34–35 | 是 | 2026-09-21 补跑（**3.4 引用这一批**） |
| `rq4_thermal_runA.csv` | **40** | 9.75 min | `-O2` | **2** | 35（恒定） | 是 | #14 第 2 次运行（20:54 起） |
| `rq4_thermal_runB.csv` | **41** | 10.0 min | `-O2` | **3→2** | 36–39 | 否 | #14 第 3 次运行（次日 05:08 起） |
...
> 补跑把它变成一个 23 分钟的实验结论：**台阶仍未触发，但延迟上升 26.9%**。
>
> **但那个 26.9% 已被 #14 推翻** —— 见上方 3.5 的警告块。两次独立重跑未复现、
> 方向相反、置换检验不显著。**RQ4 现在唯一可支持的结论是「轮间方差很大（CV 19–26%）」
> 与「热档台阶真实存在」两条。**
>
> ⚠️ **两次 #14 重跑都未做 `aa force-stop` 冷启**（运行 B 的记录显示是一次前后台切换，
> 非进程重启），因此严格说它们是「同进程内重跑」而非完全独立。
> 这是引用 3.5 时必须一并说明的限制。
