# ADR-0009 · 无人值守自动化在手机侧不可实现；读屏可做，注入点击不可

- **状态**：已接受
- **日期**：2026-09-20
- **前置**：无（独立于 LPR 主线）
- **证据**：HarmonyOS 6.1 / API 24 官方文档（OpenHarmony docs 上游 + 华为渲染页交叉核实）
...
- 本 ADR 的"PC/2in1 限定"来自**文档原文**，未在 nova 14 Pro 真机上验证是否硬性运行时门禁（文档措辞是「调用无效果」）。若要推翻，需真机实验。
- 本 ADR **不**排除通过企业/合作伙伴签名或系统应用身份实现；但那不是个人开发者路径。
- 自动化目标 App 的 ToS 风险**未评估**——即使技术可行，也可能违反服务条款。

## 未完成动作

1. 若启动自动化线：用 `hmdriver2` 搭 PC 在环最小闭环，测单动作延迟。
2. 真机验证 `OH_Input_InjectTouchEvent` 在手机上是否确实无效（可推翻本 ADR 的边界声明）。
