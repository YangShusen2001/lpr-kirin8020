# Note · 工具链与源码全部在磁盘上；CANN 侧可做真机对撞

> 本文件原为 `ADR-0008`，2026-09-20 移出 ADR 系列。原因：它记录的是**环境事实与搜索更正**，不是"难逆转 + 无上下文会惊讶 + 真实取舍"三者齐备的决策（见 `domain-modeling` 的 ADR 判据）。内容完整保留。
> 相关决策仍见 ADR-0001（继承资产）与 ADR-0004（后端选择）。
...
- 本笔记 **不**主张 `.om` 优于 `.ms`：已有实测是 MS Lite 在识别器上更快（3.99 vs 5.19 ms），CANN 的价值在**算子覆盖对照面**（166 IR 算子 vs NNRT 的 58/198 且全 FP16）。
- 本笔记 **不**主张 OMG 与设备 CANN 版本一定匹配：设备侧 `108.631.120.010` 与 DDK 的版本对应规则**无官方文档**（华为在 5.0.1.0 删除了版本↔芯片匹配表）。首次转换须以 `compat=` / `build_rc=` / `run_rc=` 三关实测为准。**本次只验证了 PC 侧转换，未在设备上跑新产的 `.om`。**
...
- 本笔记 **不**主张 `lpr-harmony` 的代码可直接复用为生产基线：它是 09-20 状态，含已知的实验档与遗留文件。
