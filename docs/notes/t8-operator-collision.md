...
>
> 后果有两层：
> 1. 相机页（T3）从 T7 起就一直坏着 —— 该缺陷在切档/跨页面并发时才发作。
> 2. **本文档的 51/51 无法在 T7 之后的树上复现**，因为那个队列修复已经不在了。
>    引用本文结果时请回到 `beb92e1` / `b40cf48`，或先确认 `napi_init.cpp` 里
>    存在 `std::deque<std::shared_ptr<Item>>`。
>
> 修复已在 `5731f5a` 重新做回并做了真机验证（见 `t3-camera-runner-drop.md`，
> 那里也记了这次回滚的机制与三条防复发纪律）。
