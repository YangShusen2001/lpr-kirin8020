...
> ⚠️ **后续更正（2026-09-21，T3 期间发现）：这个修复在 T7 提交里被静默回滚了。**
>
> T8 的修复**确实进了本轮的提交**（`beb92e1` 里 `napi_init.cpp` 是队列版），
> 但紧接着的 T7 提交把它**整份改回旧的 `has_` 版** —— 原因是 T8 排查「未定位
> 异常」时检出过 `ca034a5`（T8 之前）的 C++ 去重构建，那个调试用的检出留在了
> 工作区，随后被 T7 一并提交。blob 逐位相同即为指纹：`984e3d5` 与 `ca034a5`
> 的 `napi_init.cpp` 都是 `81d4a9f6…`，而 T8 是 `306beaaf…`。
>
> 后果：相机页（T3）从 T7 起一直坏着 —— 该缺陷在切档/跨页面并发时才发作。
>
> **可复现性已实测确认（不是推断）**：修复已在 `5731f5a` 重新做回，
> 随后在**当前 HEAD** 上重跑本探针，结果与本文原始证据**逐算子完全一致**：
>
> ```
> 重跑算子数 51 / 原始 51     逐算子差异数 0
> compat=0 : 51    construct=ok : 51
> build_rc=0 : 51  run_rc=0 : 51     空返回 0
> ```
>
> 存档：`evidence/t8_op_collide_rerun.log`（app 仓库）。
> 所以本文的 51/51 **在含修复的树上可复现**；只有 T7 那一段历史区间里不可复现。
>
> 修复的完整记录与三条防复发纪律见 `t3-camera-runner-drop.md`。
