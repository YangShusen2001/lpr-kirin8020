...
> ⚠️ **状态标注（2026-09-22 收尾）**：本文是 2026-09-21 的规划快照，§8.1 已全部执行、
> §8.2 大部分已落账。**当前状态一律以根 README 与 `evidence-index.md` 为准**，本文保留作为过程记录。
>
...
## 0.5 现状更新（2026-09-22，晚）

> 下面 §1 的审查是 **2026-09-21 会话开始时**的快照，保留原样作为记录。
> 这一节只写**从那以后发生了什么**，不改上面任何数字。

| 维度 | 09-21 快照 | 09-22 现状 |
|---|---|---|
| 论文正文 | 0%（旧稿已归档） | **英文主稿已成稿**：`paper/en/main.tex` 643 → **695** 行，tectonic 编译 **8 页**（原 7 页），零缺字/零 overfull/零 error；中文并行稿**仍未建** |
| 数字口径 | 已有 evidence-index | **守卫从 67 项扩到 91 项**，新增 T8（车辆检测上 NPU）一节的 23 项；`scan_for_publication` 0 阻断 |
| 开源交付 | ~40%，两库 private | README 中英两版均已补 T8 与许可分列；`paper/` 图片由 `tools/make_figures.py` 重新生成 |
| 对外呈现（网页/简历） | **0%** | **简历条目已补 T8**（中英、标准/精简、速查卡、一句话概括）；**网页单页设计已开工** |
| 新增实验 | — | **车辆流水线 T5–T8**：T5 界面已闭环、T6 性能实测、T8 换原版 YOLOv5 并真机落 NPU（裸头 7.79×） |

**当前最短路径**：中文稿（并行稿）→ 网页单页 → 转 public。
...
9. 落点面板在 App 内落地（ADR-0003 未完成动作 2）—— **对演示页/视频价值高**。**[仍未做]**
10. 纯前端零安装演示页（spec user story 54）—— **对求职价值最高的一项**。**[仍未做]**（`lpr-showcase` 有现成 WASM 代码可复用）
11. 网页证据仪表盘（spec user story 53）。**[仍未做]**

**本节之外、会话中新发现的缺口：**

12. **绿牌 det A/B 未跑** —— T13 §6.5 自己记的缺口。**代码已写好、只差跑**（App 里有「绿牌 det」按钮）。见 §8.2 P1。
13. **论文 Related Work 只有 5 篇** —— 投稿前需扩到 15–25 篇。
14. **论文 PDF 未编译** —— 本机无 LaTeX 发行版。
15. **中文稿未写**；**简历条目已按新主张重写**（原缺）。
...
### 8.1 已完成（2026-09-21 会话）

| 原优先级 | 事项 | 结果 |
|---|---|---|
| **P0** | `lpr-showcase` 建 git + 推私库 | ✅ 1385 文件 / 128 MiB，`YangShusen2001/lpr-showcase`（private） |
| **P0** | 定"数字一致性表"（§7.1） | ✅ `docs/evidence-index.md` + `tools/verify_published_numbers.py`（**60 项机检**） |
| **P0** | 补跑 RQ4 到 20 分钟 / 80 轮 | ✅ 80 轮 / 23.30 分钟。**台阶仍未触发，但延迟漂移 +26.9%** |
| **P1** | 出英文稿（IEEEtran conference） | ✅ `paper/en/main.tex`，8 节 |
| **P1** | 画图 | ✅ 6 张（PNG + PDF），由 `tools/make_figures.py` 从证据生成 |
| **P1** | 旗舰仓库 README + 合规清理 | ✅ 中英双语 README；合规扫描两个仓库 **0 阻断**（**尚未转 public**） |
| **P2** | 简历条目 | ✅ `docs/resume-bullets.md`，中英双语，取代旧主张版 |

### 8.2 剩余

| 优先级 | 事项 | 成本 | 说明 |
|---|---|---|---|
| **P0** | **论文 PDF 编译** | 1–2 小时 | 唯一还卡着的技术步骤。本机无 LaTeX 发行版；`.tex` 与 6 张图（PNG+PDF）都已就位，用 `latex-paper-tectonic` 技能可直接出 PDF |
| **P0** | **决定是否转 public** | 你拍板 | 合规清理已完成（0 阻断）。**转 public 不可逆**（fork 与缓存会留存），所以不由我代决 |
| **P1** | **绿牌 det A/B** | 半天 | **代码已写好、只差跑**：App 里已有「绿牌 det」按钮，调 `ccpdSceneAB('det','green')`。T13 §6.5 明确记着这个缺口 —— 8 字符上 det 落点是否仍像 7 字符那样 0.1%，**没测** |
| **P1** | Related Work 扩写 | 1–2 天 | 目前只有 **5 篇**（逐条核验过）。投稿需扩到 15–25 篇，并补「移动端 NPU 异构调度」与「算子委托/回落」两条线的文献 |
| **P1** | 中文稿 | 数天 | 按「重写不翻译」的原则（§3.3）。目标刊先定，再按该刊投稿指南核模板与篇幅 |
| **P2** | 个人项目页 | 2–3 天 | 单页。最该突出的是**自我更正时间线**（T10→T12、T11→T14、ADR-0003/0005） |
| **P2** | 零安装 Demo | 1–2 天 | `lpr-showcase` 里有现成的纯前端 WASM 演示代码可复用 |
| **P2** | 投稿目标选择 | — | 路径 A：arXiv 预印本（一周内可上线，直接可用于求职）；路径 B：英文期刊/会议；路径 C：中文刊。建议**先 A 再决定 B/C** |
| **P3** | 融合携带「相关 → 因果」 | 数天 | 同权重、仅改变算子孤立性的对照实验（ADR-0002 未完成动作 1）。审稿人最可能要求补的一条 |
| **P3** | RQ2 跨 SKU | 不可控 | 依赖门店演示机，已按 ADR-0007 降级为未来工作 |
| — | `libncnn.so` / `libomp.so` | 你决定 | 被跟踪。再分发通常允许但需署名 —— 随仓库发布，还是改为文档化获取步骤？ |
| — | 双层黄牌 / 港澳两地牌 | 可选 | ADR-0006 未完成动作 1；如实写"未测"即可，不阻断 |

### 8.3 一句话优先级

**先编译 PDF、再决定 public、然后跑绿牌 det。** 前两件是"把已完成的成果变成可用形态"，
第三件是"用半天换一个已写好的实验缺口"。中文稿与网页都可以在 arXiv 预印本上线之后再做。

> 注意：本文 §1.3 与 §8.1 的"完成度"判断已按实际执行结果更新。
> 原 §8 把 P0/P1 列为待办，现已全部完成 —— 保留本节作为执行记录。
